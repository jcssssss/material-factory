...
export default function LogViewerPage() {
  ...
  return (
    <div className="flex h-full flex-col gap-5 p-6">
      ...
      <div className="flex-1 overflow-hidden rounded-xl border border-workspace-border/60 bg-workspace-surface shadow-card">
        {filtered.length === 0 ? (
          ...
          <ul className="h-full divide-y divide-workspace-border/20 overflow-auto">
            {filtered.map((log, idx) => (
              <li key={idx} className="flex items-start gap-3 px-4 py-2.5 text-xs transition-colors hover:bg-slate-50/50">
                <span className="mt-0.5 shrink-0 font-mono text-workspace-muted">
                ...
                <span className="min-w-0 flex-1 break-words font-mono text-workspace-fg-secondary">
                  {log.message}
                </span>
              </li>
            ))}
