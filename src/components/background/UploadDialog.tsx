...
import { saveBackgroundFile, addTemplate } from "../../lib/printEngine/backgroundDb";
...
export default function UploadDialog({ open, onClose }: Props) {
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      const fileName = await saveBackgroundFile(Array.from(bytes), ext);
      ...
      const id = await addTemplate(fileName, img.naturalWidth, img.naturalHeight, file.size);
      onClose();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-2xl bg-workspace-surface p-6 shadow-2xl">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-sm font-semibold text-workspace-fg">上传背景模板</h2>
          <button
            type="button"
            onClick={handleClose}
        ...
        {!previewUrl ? (
          ...
          >
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="mb-2 h-8 w-8 text-workspace-muted/50">
              ...
              <path fillRule="evenodd" d="M2.5 16.5a.75.75 0 01.75-.75h13.5a.75.75 0 010 1.5H3.25a.75.75 0 01-.75-.75z" clipRule="evenodd" />
            </svg>
            <p className="text-xs text-workspace-muted">点击选择图片</p>
            <p className="text-[10px] text-workspace-muted/60">支持 JPG / PNG</p>
            <input
              ref={inputRef}
              type="file"
          ...
          <div className="space-y-3">
            <div className="overflow-hidden rounded-lg border border-workspace-border/60 bg-slate-100">
              <img src={previewUrl} alt="预览" className="mx-auto max-h-64 object-contain" />
            </div>
